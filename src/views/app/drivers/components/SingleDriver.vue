  ...
  >
    <div class="space-y-7 lg:p-8 p-7">
      ...
      <main class="flex flex-col space-y-2 justify-center items-center">
        <img v-if="driverData.avatar" :src="driverData.avatar" class="rounded-full h-20 w-20 object-cover" />
        <span v-else class="rounded-full p-2 text-gray-100 font-medium bg-gray-400">{{ driverData.fname.slice(0, 1)}}{{ driverData.lname.slice(0, 1) }}</span>
      </main>
        <p class="font-medium text-gray-600 text-center">{{driverData.fname}} {{driverData.lname}}</p>
      <div>
      </div>
    </div>
      ...
      <div class="flex justify-between items-center pb-6">
        <div class="flex justify-start space-x-2">
          ...
          <div class="">
            ...
            <p class="font-medium text-xs">{{driverData.assigned_vehicle ? driverData.assigned_vehicle : 'N/A'}}</p>
          </div>
        </div>
      ...
      <div class="flex justify-between items-center py-6">
        <div class="flex space-x-2">
          ...
          <div class="">
            ...
            <p class="font-light text-xs"> {{driverData.total_trips ? driverData.total_trips : 'N/A'}}</p>
          </div>
        </div>
    ...
    <div class="flex justify-between items-center py-6">
        <div class="flex space-x-2">
          <img class="h-5 w-5" src="@/assets/images/email.svg" />
          <div class="">
