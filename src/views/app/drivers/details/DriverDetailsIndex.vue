<template>
  <page-layout :page-title="!loading && !isLoading && driverData && driverData.brand ? `${driverData.brand} ${driverData.name}` : 'Driver Information'">
      ...
      <page-action-header>
        ...
        <template #tabs>
          <router-link class="text-sm font-medium leading-6 pb-2 pt-1 px-2 border-b cursor-pointer" active-class="text-black border-b-sh-green-500 border-b-2" to="information">
            Driver information</router-link>
...
export default {
  ...
  created () {
    this.$store.dispatch('driver/fetchDriverInfo', this.$attrs.driverId).finally(() => {
      this.loading = false
    });
  },
}
