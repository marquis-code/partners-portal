    ...
    />
     ...
     <div v-if="!fileUploaded" class="space-y-2 w-full relative">
        ...
        <div class="flex flex-col w-full">
            ...
            <div class="doc-info-and-delete flex flex-row justify-between items-center">
                <div class="bg-green-500 w-10/12 h-2 rounded-2xl"></div>
                <p class="w-2/12 text-right">100%</p>
            </div>
        </div>
    </div>
    <p v-if="fileUploaded" @click="$refs.avatar.click()" class="text-sh-purple-700 font-medium text-sm mb-4">Change Document</p>
  </div>
</template>
